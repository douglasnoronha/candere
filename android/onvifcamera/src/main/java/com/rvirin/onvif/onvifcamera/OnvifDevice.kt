    ...
    enum class Type {
        ...
        fun namespace(): String {
            when (this) {
                ...
                GetProfiles, GetStreamURI, GetSnapshotURI -> return "http://www.onvif.org/ver20/media/wsdl"
            }
        }
    }
}
 ...
 * ## make sure you run this is a thread ##
...
class OnvifDevice(val ipAddress: String, @JvmField val username: String, @JvmField val password: String) {
    ...
    fun getDeviceInformation(): OnvifResponse {
        val request = OnvifRequest(deviceInformationCommand, OnvifRequest.Type.GetDeviceInformation)
        return ONVIFcommunication().execute(request).get()
    }

    fun getProfiles(): OnvifResponse {
    ...
    fun getStreamURI(): OnvifResponse {
        val profile = mediaProfiles.firstOrNull()!!
